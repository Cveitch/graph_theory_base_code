...
class GraffWriter:
    ...
    def update_graff_writer_colourmap(self, nsize, group):
        ...
        nc = nx.draw_networkx_nodes(self.my_graff.G, pos, nodelist=nods, node_color=colours,
                                    with_labels=False, node_size=nsize, cmap=plt.cm.jet)
        lc = nx.draw_networkx_labels(self.my_graff.G, pos)
        plt.colorbar(nc)
        plt.axis('off')
        plt.show()

    def plot_graff(self, nsize, spectral=True, colourmap=False, group=None):
        if colourmap:
            self.update_graff_writer_colourmap(nsize, group)
        else:
            if spectral:
